...
const showBotHandoffBanner = computed(
  () =>
    (isSimpleMode.value || isUserTyping.value) &&
...
const selfAssignBannerMessage = computed(() => {
  return isSimpleMode.value
    ? t('CONVERSATION.SIMPLE_NOT_ASSIGNED_TO_YOU')
    : t('CONVERSATION.NOT_ASSIGNED_TO_YOU');
});

const selfAssignActionLabel = computed(() => {
  return isSimpleMode.value
    ? t('CONVERSATION.ASSUME_ATTENDANCE')
    : t('CONVERSATION.ASSIGN_TO_ME');
});
...
<template>
  <Banner
    ...
    :banner-message="selfAssignBannerMessage"
    has-action-button
    :action-button-label="selfAssignActionLabel"
