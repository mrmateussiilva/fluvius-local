...
const defaultEditorHeight = computed(() =>
  isSimpleAgentMode.value ? SIMPLE_MODE_DEFAULT_HEIGHT : DEFAULT_HEIGHT
);
const editorHeight = ref(defaultEditorHeight.value);
...
const sizeBounds = computed(() => {
  ...
  return {
    min: MIN_HEIGHT,
    max: isContainerReady.value ? max : defaultEditorHeight.value,
    expanded,
    default: clamp(defaultEditorHeight.value, MIN_HEIGHT, max),
  };
});
...
watch(defaultEditorHeight, (height, previousHeight) => {
  if (editorHeight.value === previousHeight) {
