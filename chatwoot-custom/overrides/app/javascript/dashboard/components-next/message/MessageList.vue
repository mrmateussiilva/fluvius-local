...
const normalizedSearchResultIds = computed(() =>
  props.searchResultIds.map(id => Number(id))
);

const isSearchResult = messageId =>
  normalizedSearchResultIds.value.includes(Number(messageId));
  ...
  <ul class="px-4 bg-n-surface-1">
    ...
    <template v-for="(message, index) in allMessages" :key="message.id">
      ...
      <Message
        ...
        :is-search-result="isSearchResult(message.id)"
        :is-active-search-result="
          Number(activeSearchResultId) === Number(message.id)
        "
